...
public class Rent implements Initializable {
    ...
    public void initialize(URL url, ResourceBundle rb) {
        ...
        int id = 0;
        String sql = "SELECT id FROM products_rent";
        try {
    ...
    private void handleBuy(ActionEvent event){
        ...
        String sql1 = "SELECT * FROM products_rent WHERE id = " + item;
        try{
            preparedStatement = connection.prepareStatement(sql1);
            resultSet = preparedStatement.executeQuery();
            while (resultSet.next()) {
                print = "" + resultSet.getString("pname");
                String sql2 = "DELETE FROM products_rent WHERE id = " + item  ;
                String sql3 = "insert into rent (name, item)" + " values (?, ?)";
                ...
                try{
                   ...
                   /* if (result !=0){
                    ...
                    infoBox("Product " + print + " Taken on Rent", "Success", null);
                }
                catch (Exception e){
                    e.printStackTrace();
